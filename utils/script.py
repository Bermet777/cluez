...
from search.lang_models import LanguageModels
from search.helpers import sent_to_words, text_from_html

gc.collect()
lm = LanguageModels()


def search(input, lang):
    log = pd.DataFrame({
    ...
    search_input = '"%s" AND ' % input
    key_out = pd.DataFrame(
    ...
    time_log = pd.DataFrame(columns=["subject", "time", "results"])

    keys = lm.keywords[lm.keywords["language"] == lang]["keywords"].values[0]
    ...
    if (response["tasks"][0]["result"][0]["items"] is None) or (response["tasks"][0]["result"] is None):
        ...
        print(INPUT + " - Response error, please see the key_out", datetime.now())
        ...
        if len(texx) == 0:
            ...
            for o in range(1):
                ...
                key_table.iloc[o, 5] = INPUT
                key_table.iloc[
                    o, 6] = "No results error. For more details see check url. It could happen if only response error recieved from sties or there are only linkedin results. They usually filter user agents and make every webscrapping stop"
                key_table.iloc[o, 7] = response["tasks"][0]["result"][0]['check_url']
                key_table.iloc[o, 15] = 0
            # key_table=key_table[key_table.iloc[:,key_table.shape[1]-1]==1].reset_index()
            key_table = key_table.rename(columns={key_table.columns[15]: 'Full name match'})
            key_out = key_out.append(key_table, ignore_index=True)
            print(INPUT + " - No results error. ", datetime.now())
            ...
            ji[0] = ji[0][1:]
            ji.append(INPUT)
            target = re.sub('"', '', lm.keywords[lm.keywords["language"] == LANG]["keywords"].values[0]).split(
                " OR ")
            target[0] = target[0][1:]
            target.append(INPUT)
            ...
            time_log.loc[0] = (INPUT, (
                log[log.step == "keytable ok"].time[9] - log[log.step == "Start Time"].time[0]).total_seconds(),
                               len(texx))

    time_log.to_csv("time_log2.csv")
    key_out.to_csv("key_out2.csv")


if __name__ == '__main__':
    INPUT = "AMC Networks CE"
    LANG = "en"
    search(INPUT, LANG)
